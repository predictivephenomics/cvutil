...
cv::Mat pdist2(std::vector<int> pts, cv::Size sz)
{
    ...
    mr = cvutil::floor(m / sz.width);
    mc = m - cvutil::floor(m / sz.width) * sz.width;
...
cv::Mat colorimage(cv::Mat img, std::vector<std::vector<int>> cc, std::vector<int> colorcodes, std::vector<std::vector<int>> colors)
{
    int c = img.cols + 2;
    ...
    for (int i = 0; i < cc.size(); i++)
    {
        for (int j = 0; j < cc[i].size(); j++)
        {
            circle(result, { ((cc[i][j] % c) - 1), ((cc[i][j] / c) - 1) }, 1, cv::Scalar(colors[colorcodes[i] - 1][2], colors[colorcodes[i] - 1][1], colors[colorcodes[i] - 1][0]));
        }
    }
